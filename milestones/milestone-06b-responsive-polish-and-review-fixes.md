...
- [x] Fix finding #90 — Update the Cosmos DB init catch block in `src/server/index.ts` to log a more prominent warning: `"⚠️  Cosmos DB initialization failed:"` followed by `"⚠️  Project CRUD endpoints will return 500 errors until Cosmos DB is available."`
