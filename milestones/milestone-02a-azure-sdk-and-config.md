...
- [x] Install Azure SDKs: `npm install @azure/storage-blob @azure/cosmos @azure/identity`
- [x] Create stamp config module at src/server/config.ts — read `STAMP_ID` from `process.env` (default `"qqq"`), export `stampId` string, `storageAccountName` as `stautodev${stampId}`, and `cosmosAccountName` as `cosmos-autodev-${stampId}`
