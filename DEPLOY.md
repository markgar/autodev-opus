...
14. **Sample specs CRUD with in-memory fallback:** The `/api/sample-specs` endpoints (GET list, GET by name, POST, DELETE) use Azure Blob Storage when available. When Blob Storage init fails at startup, the service falls back to an in-memory store (see note #40). Data does not persist across restarts in fallback mode. Set `AZURE_STORAGE_CONNECTION_STRING` to use Azurite for local development with persistence.
...
17. **Sample specs API without Azure:** When Azure Blob Storage is unavailable, `GET /api/sample-specs` returns HTTP 200 with data from the in-memory fallback store (see note #40). On first startup without Azure, this is an empty array, so the SampleSpecsPage shows the "No sample specs uploaded yet" empty state. Specs uploaded in-memory are available until the server restarts.
...
24. **Projects API with in-memory fallback:** When Cosmos DB is unavailable, `GET /api/projects` returns HTTP 200 with data from the in-memory fallback store (see note #40). `GET /api/projects/:id` returns 404 for unknown IDs or 200 for in-memory projects. Data does not persist across restarts in fallback mode.
...
35. **Spec name validation:** Routes validate spec names with `isValidSpecName()` before any CRUD operation. Invalid names return 400 `{ message: "Invalid spec name" }`. Valid pattern: `/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,251}\.md$/` (max 255 chars total) and no `..` sequences.
