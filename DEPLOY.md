...
24. **Playwright container restart:** If the playwright container dies (OOM or timeout during first test run), restart with `docker compose up -d playwright`, then re-copy test files before retrying.
25. **Playwright test file naming:** E2E test files follow `e2e/milestone-{id}.spec.ts` naming pattern. Copy files into playwright container with `docker cp` due to DinD volume mount limitations.
26. **Logs API leaks credentials:** `GET /api/projects/:id/logs` returns the raw Azure error message in the 500 response body (unlike `GET /api/projects/:id` which returns generic "Internal server error"). See issue #122.
27. **ProjectDetailPage requires mocked API for Playwright:** Without Azure, project detail page always shows error state. Use `page.route()` to mock both `/api/projects/:id` and `/api/projects/:id/logs` to test full UI behavior (heading, log viewer, pause/resume, back link).
