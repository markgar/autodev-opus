...
31. **POST /api/projects route:** Validates `name` (non-empty string, max 100 chars) and `specName` (non-empty string) before calling Cosmos DB. Returns 400 with `{ "message": "..." }` on validation failure, 201 with full project JSON on success, 500 on Cosmos/Blob failure.
...
31. **RestError propagation:** `sampleSpecs.ts` service uses `RestError` `statusCode` checking (not string matching) for 404 detection. Azure SDK errors propagate correctly.
